...
| [s24-log-command.md](core/s24-log-command.md) | §24 The `ail log` Command | Single-run inspection; `--format` and `--follow` flags; exit codes; project scoping | **alpha** — fully documented |
| [s25-logs-command.md](core/s25-logs-command.md) | §25 The `ail logs` Command | Multi-session listing; `--session`, `--query`, `--tail`, `--limit`, `--format`; FTS search; JSON output schema | **alpha** — fully documented |
...
| [s32-init-command.md](core/s32-init-command.md) | §32 The `ail init` Command | Bundled-template workspace scaffolding; install rule (`.ail/` under CWD); picker + non-TTY behaviour; conflict semantics (`--force`, `--dry-run`); manifest schema; extension points (`TemplateSource` trait) | **v0.3** — `ail-init` crate; three bundled templates (starter, superpowers, oh-my-ail) |
| [s33-url-template-source.md](core/s33-url-template-source.md) | §33 URL-Based Template Source | `ail init <URL>`; GitHub shorthand + web URL + raw manifest URL grammar; manifest + per-file fetch pipeline; security envelope; byte / count caps; `ail:init/failed` error surface | **v0.4** — `UrlSource` in `ail-init`; mockito-backed tests; no registry / auth / cache in v1 |
...
| [r06-codex-runner.md](runner/r06-codex-runner.md) | r06. Codex Runner | OpenAI Codex CLI wrapper; non-interactive `codex exec --json` mode | draft |
