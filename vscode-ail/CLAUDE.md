...
vscode-ail/
  ...
  src/
    ...
    panels/
      ExecutionPanel.ts   # WebviewPanel (editor column): step list, streaming, cost bar
  ...
  └─ ServiceContext(ctx, outputChannel, client)
       └─ EventBus
            └─ RunnerService(services, bus)
                 ...
                 └─ ExecutionPanel (created per run)
```

### Event Two-Tier System

**Tier 1 — `AilEvent` (src/types.ts):** Full-fidelity wire format. Mirrors the Rust NDJSON output exactly. Includes thinking, tool_use, tool_result, cost_update, permission_requested events. Used by `ExecutionPanel` (raw fidelity needed for UI rendering).
...
4. Each `AilEvent` → `_emitRaw()` → raw handlers (ExecutionPanel)
...
- Use `RunnerEvent` in application layer; only `ExecutionPanel` needs `AilEvent`
- `resolvePipelinePath()` is the single source of truth for pipeline discovery

## Known Issues / Planned Work

See implementation plan at `~/.claude/plans/woolly-plotting-bird.md` for the full Phase 0–4 roadmap.

Current known gaps (not yet implemented):
- **Phase 1.1:** `parseStepsFromYaml()` in `pipeline.ts` uses regex — should use `yaml` npm package
- **Phase 1.2:** Rust event stream lacks `resolved_prompt` and `response` fields
- **Phase 2:** No persistent run history; no Stage panel (ExecutionPanel is ephemeral)
- **Phase 3:** Bidirectional stdin protocol not implemented (blocks HITL gate UI)
- `RunnerService.ts` has a dead import of `parseNdjsonStream` (used in AilProcess, not here)
